...
from ..models.schemas import (
    TestRun, TestRunCreate, TestResult, TestReportOverview,
    TestReportDetail, VersionComparison, TestProgress, MessageResponse
)
...
async def start_test_run(test_run: TestRunCreate):
    test_run_id = await test_engine.start_test_run(test_run.name, test_run.description, test_run.batch_id)
    return get_test_run(test_run_id)


@router.get("/{test_run_id}/progress", response_model=TestProgress)
def get_test_progress(test_run_id: str):
...
def get_test_results(test_run_id: str, category: str = None):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        if category:
            cursor.execute("SELECT * FROM test_results WHERE test_run_id = ? AND category = ?", (test_run_id, category))
        else:
...
def get_test_report(test_run_id: str):
...
def export_test_results_excel(test_run_id: str):
    test_run = get_test_run(test_run_id)
    results = get_test_results(test_run_id)
    ...
    try:
        ...
        return FileResponse(
            ...
            filename=f"测试结果_{test_run.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
...
def export_test_results_csv(test_run_id: str):
    test_run = get_test_run(test_run_id)
    results = get_test_results(test_run_id)
...
def get_test_logs(test_run_id: str, question_id: str = None, log_type: str = None):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        query = "SELECT * FROM test_logs WHERE test_run_id = ?"
        params = [test_run_id]
...
def compare_test_runs(base_run_id: str, compare_run_id: str):
    base_report = get_test_report(base_run_id)
    compare_report = get_test_report(compare_run_id)

    score_change = compare_report.overview.average_score - base_report.overview.average_score

    category_changes: Dict[str, float] = {}
    all_categories = set(base_report.overview.category_stats.keys()) | set(compare_report.overview.category_stats.keys())
    ...
    base_low_scores = [
        {"question_id": r.question_id, "question": r.question, "score": r.total_score}

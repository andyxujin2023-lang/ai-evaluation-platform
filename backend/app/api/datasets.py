...
from ..models.schemas import TestQuestion, TestQuestionCreate, TestQuestionUpdate, MessageResponse
...
def list_questions(category: str = None, batch_id: str = None):
...
def list_categories():
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT category FROM datasets")
        rows = cursor.fetchall()
...
def import_questions(questions: List[TestQuestionCreate]):
    ...
    with get_db_connection() as conn:
        ...
        for q in questions:
            ...
            cursor.execute("""
                INSERT INTO datasets (id, batch_id, question, standard_answer, keywords, category, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                question_id,
                q.batch_id,
...
async def import_excel(file: UploadFile = File(...), batch_id: str = None):
...
def get_question(question_id: str):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM datasets WHERE id = ?", (question_id,))
        row = cursor.fetchone()
        if not row:
...
def create_question(question: TestQuestionCreate):
    question_id = str(uuid.uuid4())
    now = datetime.now().isoformat()
...
def update_question(question_id: str, question: TestQuestionUpdate):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM datasets WHERE id = ?", (question_id,))
        ...
        if updates:
            ...
            cursor.execute(f"""
                UPDATE datasets SET {', '.join(updates)} WHERE id = ?
            """, params)
...
def delete_question(question_id: str):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM datasets WHERE id = ?", (question_id,))
...
def batch_delete_questions(request: BatchDeleteRequest):
    if not request.ids:
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(f"DELETE FROM datasets WHERE id IN ({placeholders})", request.ids)
